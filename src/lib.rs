...
mod tests {
    ...
    pub const TYPE2_REGULARIZATION: f64 = 0.0;
    pub const SAFEGUARD_TOLERANCE: f64 = 2.0;
    pub const STEPSIZE: f64 = 0.01;
    pub const MAX_AA_NORM: f64 = 1e10;
    pub const ITERS: usize = 10000;
    ...
    fn rand_q(dim: usize) -> Array2<f64> {
        ...
        q.diag_mut().add_assign(1e-2);
        q
    }

    /// Compute the gradient descent step:
    /// F (x) = x - d * Q * x, where d is the stepsize
    fn f(x: ArrayView1<f64>, q: ArrayView2<f64>) -> Array1<f64> {
        &x - STEPSIZE * q.dot(&x)
    }

    fn gradient_descent(aa_type: AAType, relax: f64) {
        let mut x = rand_x(DIM);
        let q = rand_q(DIM);
        let mut err = f64::NAN;
        ...
        let mut aa = AndersonAccelBuilder::default()
            .dim(DIM)
            .memory(MEM)
            .aa_type(aa_type)
            .beta(relax)
            .safeguard_factor(SAFEGUARD_TOLERANCE)
            .regularization(reg)
            .max_weight_norm(MAX_AA_NORM)
            .build()
            .unwrap();

        for _ in 0..ITERS {
            x = aa.apply(x.view(), y.view()).unwrap();
            y = f(x.view(), q.view());
            err = x.norm_l2();
        }
        assert!(err < 1e-6);
    ...
    #[test]
    fn gd_type2_relax1() {
        gradient_descent(AAType::II, 1.0);
    }
